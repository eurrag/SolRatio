"""
solratio_core.py  |  SolRatio v4.1.2 (2026-05-01)
====================================================
...
def compute_post_shadow(x_pts, df, p):
    """
    Calcola la matrice f_palo(n_times x n_points):
      Valore frazionario in [0, 1] = frazione del tempo in cui il punto è
      ombreggiato dal palo lungo la direzione N-S.

    v2.4.1: il palo proietta una STRISCIA d'ombra da base (sull'asse tracker)
    alla proiezione della cima, non un disco centrato sulla proiezione.

    Il palo è verticale, sezione d_palo, posizionato sugli assi dei tracker.
    L'ombra trasversale (E-O) è calcolata dalla posizione solare.

    f_palo viene usato nel chiamante come:
      f_dir = where(f_dir > 0, f_dir * (1 - f_palo), f_dir)
    L'ombra del palo è applicata SOLO ai punti illuminati (f_dir > 0),
    evitando doppio conteggio con l'ombra del pannello.

    Se d_palo = 0, restituisce matrice di zeri (pali disattivati).
    """
    n_times  = len(df)
    n_points = len(x_pts)
    ...
    if d_palo <= 0:
        return np.zeros((n_times, n_points))

    H        = p['H']
    pitch    = p['pitch']
    spaz     = p.get('spaziatura_pali', 5.0)
    tan_s    = np.tan(p.get('slope_cross_rad', 0.0))

    # Fattore di copertura longitudinale: il palo copre d_palo su ogni
    # tratto di spaziatura_pali lungo l'asse N-S.
    cover_ratio = min(d_palo / spaz, 1.0) if spaz > 0 else 1.0

    sun_up = df['apparent_elevation'].values > 2.0
    elev   = df['apparent_elevation'].values
    azim   = df['azimuth'].values

    # Vettorizzazione: calcola posizione ombra per tutte le ore
    elev_rad = np.radians(elev)
    azim_rad = np.radians(azim)
    tan_elev = np.maximum(np.tan(elev_rad), 0.01)

    # Componente trasversale del raggio solare (perpendicolare all'asse tracker).
    # Per axis_azimuth=180 (N-S): sin(az - 180) = componente E-O (identico a v2.4).
    # Per axis_azimuth arbitrario: proietta sul piano perpendicolare all'asse.
    ax_az_rad = np.radians(p.get('axis_azimuth', 180.0))
    sin_az_cross = np.sin(azim_rad - ax_az_rad)                   # (n_times,)

    # Semi-larghezza dell'ombra trasversale del palo al suolo
    shadow_width = d_palo / np.maximum(np.sin(elev_rad), 0.05)  # (n_times,)
    half_w = shadow_width / 2.0

    xp = np.array(x_pts)                                        # (n_points,)

    # Assi dei pali: tracker vicini
    axes = np.array([i * pitch for i in range(-1, 3)])

    # Inizializza
    f_palo = np.zeros((n_times, n_points))

    # Indici ore diurne
    t_sun = np.where(sun_up)[0]
    if len(t_sun) == 0:
        return f_palo

    hw = half_w[t_sun]                                           # (n_sun,)
    sin_cross = sin_az_cross[t_sun]                                    # (n_sun,)
    te = tan_elev[t_sun]                                         # (n_sun,)

    f_palo_sun = np.zeros((len(t_sun), n_points))

    for axis in axes:
        # Quota della cima del palo sopra il suolo locale
        # Su terreno inclinato: z_cima = z_ground(axis) + H = axis*tan_s + H
        # La proiezione dell'ombra al suolo (piano inclinato) è:
        #   x_sh = axis + (axis*tan_s + H) * sin(az-π) / (tan_elev + tan_s*sin(az-π))
        # Su terreno piano (tan_s=0) si riduce a: axis + H*sin(az-π)/tan_elev
        z_cima = axis * tan_s + H
        denom = te + tan_s * sin_cross                              # (n_sun,)
        safe_denom = np.where(np.abs(denom) > 0.001, denom, 1e6)
        x_shadow = z_cima * sin_cross / safe_denom                 # (n_sun,)
        sh_end = axis + x_shadow                                 # (n_sun,)

        sh_min = np.minimum(axis, sh_end)[:, None]               # (n_sun, 1)
        sh_max = np.maximum(axis, sh_end)[:, None]               # (n_sun, 1)

        # Il punto è in ombra se cade nella striscia ± semi-larghezza
        hit = ((xp[None, :] >= sh_min - hw[:, None]) &
               (xp[None, :] <= sh_max + hw[:, None]))            # (n_sun, n_points)

        # Il valore è il cover_ratio (peso longitudinale)
        f_palo_sun = np.where(hit, np.maximum(f_palo_sun, cover_ratio), f_palo_sun)

    f_palo[sun_up] = f_palo_sun
    return f_palo


# ══════════════════════════════════════════════════════════════════════════════
# DECOMPOSIZIONE DIFFUSA — Perez et al. (1990)
# ══════════════════════════════════════════════════════════════════════════════
...
# (usata da main, solratio_edge, optimize_pitch, benchmark)
# ══════════════════════════════════════════════════════════════════════════════
...
def optimize_pitch(df, p_base, pitch_range=None, step=0.25, target_crop='foraggere',
                   target_months=None):
    """
    Calcola K_agv(pitch) per una serie di pitch e trova l'ottimale.

    v3.3.0: usa decomposizione Perez (pre-calcolata fuori dal loop, nessun
    overhead). target_months allineato a Mar-Set per coerenza con Riepilogo.

    Parametri:
      df         : DataFrame completo con posizione solare e irradianza
      p_base     : dict parametri base (da read_parameters)
      pitch_range: (min, max) in metri. Default: (W+0.5, 3*W)
      step       : incremento pitch in metri (default 0.25)
      target_crop: chiave coltura per K_agv (default 'foraggere')
      target_months: lista mesi per media K_agv (default [3..9] = Mar-Set)

    Restituisce dict:
      {
        'pitches':     array di pitch testati,
        'kagv_sau':    array di K_agv SAU per ogni pitch,
        'kagv_centr':  array di K_agv Centrale per ogni pitch,
        'par_rel_sau': array di PAR relativa SAU per ogni pitch,
        'gcr':         array di GCR per ogni pitch,
        'optimal_pitch': pitch con K_agv SAU massimo,
        'crop_key':    coltura usata,
      }
    """
    W = p_base['W']
    if pitch_range is None:
        pitch_range = (W + 0.5, 3.0 * W)
    if target_months is None:
        target_months = [3, 4, 5, 6, 7, 8, 9]  # Mar-Set, coerente con Riepilogo

    pitches = np.arange(pitch_range[0], pitch_range[1] + step/2, step)
    n_points = max(3, p_base.get('n_points', 51))

    # Filtra solo i mesi target per velocità
    month_mask = df.index.month.isin(target_months)
    sun_mask = df['apparent_elevation'].values > 2.0
    combined_mask = month_mask & sun_mask
    df_sub = df[combined_mask].copy()

    if len(df_sub) == 0:
        print('   AVVISO: nessun dato solare nei mesi target.')
        return None

    theta_sub = np.radians(df_sub['tracker_theta'].values)
    tau = p_base.get('tau', 0.0)

    # Irradianza per DLI di riferimento (cielo aperto)
    # v3.3.1: usa cos_incidence (formula unificata piano/slope, coerente col main)
    dni = df_sub['dni'].values
    dhi = df_sub['dhi'].values
    slope_tilt_rad = np.radians(p_base.get('slope_angle', 0.0))
    slope_aspect_rad = np.radians((p_base.get('slope_azimuth', 0.0) + 180.0) % 360.0)
    elev_rad_opt = np.radians(np.clip(df_sub['apparent_elevation'].values, 0, 90))
    az_rad_opt = np.radians(df_sub['azimuth'].values)
    cos_incidence_opt = np.maximum(0.0,
        np.sin(elev_rad_opt) * np.cos(slope_tilt_rad) +
        np.cos(elev_rad_opt) * np.sin(slope_tilt_rad) *
        np.cos(az_rad_opt - slope_aspect_rad))
    dni_ground_opt = dni * cos_incidence_opt
    ghi = df_sub['ghi'].values if 'ghi' in df_sub.columns else (dni_ground_opt + dhi)
    albedo = p_base.get('albedo', 0.23)

    # Decomposizione Perez (v3.3.0): pre-calcolata una sola volta
    # Le 3 componenti non dipendono dal pitch → nessun overhead nel loop
    dhi_circ_opt, dhi_horiz_opt, dhi_iso_opt = compute_perez_components(df_sub, p_base)

    # DLI riferimento (senza pannelli): Perez-consistent, PAR_FRAC variabile
    dni_extra_opt = pvlib.irradiance.get_extra_radiation(df_sub.index.dayofyear)
    if isinstance(dni_extra_opt, pd.Series):
        dni_extra_opt = dni_extra_opt.values
    cos_z_opt = df_sub['cos_zenith'].values if 'cos_zenith' in df_sub.columns else \
                np.maximum(0, np.cos(np.radians(90 - df_sub['apparent_elevation'].values)))
    par_frac_opt = compute_par_frac(ghi, dni_extra_opt, cos_z_opt)
    PAR_ref = (dni_ground_opt + dhi_circ_opt + dhi_iso_opt + dhi_horiz_opt) * par_frac_opt * W_TO_UMOL * 3600 / 1e6
    dates_sub = df_sub.index.normalize()
    dli_ref_daily = pd.Series(PAR_ref, index=df_sub.index).groupby(dates_sub).sum()
    dli_ref_mean = dli_ref_daily.mean()

    results = {
        'pitches': pitches,
        'kagv_sau': np.full(len(pitches), np.nan),
        'kagv_centr': np.full(len(pitches), np.nan),
        'par_rel_sau': np.full(len(pitches), np.nan),
        'gcr': np.full(len(pitches), np.nan),
        'crop_key': target_crop,
    }

    n_pitches = len(pitches)
    for pi, pitch in enumerate(pitches):
        print(f'   Pitch {pi+1}/{n_pitches}: {pitch:.2f}m  ', end='\r', flush=True)
        # Costruisci parametri per questo pitch
        p_i = p_base.copy()
        p_i['pitch'] = pitch
        p_i['GCR'] = W / pitch
        p_i['SAU'] = pitch - 2 * p_i['sanu']
        if p_i['SAU'] <= 0:
            continue

        x_pts_i = np.linspace(0, pitch, n_points)

        # Ricalcola tracker theta per il nuovo GCR (v3.1 fix)
        tracker_i = tracking.singleaxis(
            apparent_zenith  = df_sub['apparent_zenith'],
            solar_azimuth    = df_sub['azimuth'],
            axis_tilt=0, axis_azimuth=p_base.get('axis_azimuth', 180.0),
            max_angle=p_i['beta_max'],
            backtrack=bool(p_i.get('backtracking', 1)),
            gcr=p_i['GCR'],
        )
        theta_i = np.radians(tracker_i['tracker_theta'].fillna(0).values)

        # VF e shadow per questo pitch con theta corretto
        VF_i = compute_vf_matrix(x_pts_i, theta_i, p_i)
        df_sub_i = df_sub.copy()
        df_sub_i['tracker_theta'] = tracker_i['tracker_theta'].fillna(0).values
        f_dir_i = compute_shadow_matrix(list(x_pts_i), df_sub_i, p_i)

        # Applica tau
        if tau > 0:
            f_dir_i = np.where(f_dir_i < 1.0, np.maximum(f_dir_i, tau), f_dir_i)

        # NOTA: l'ombra pali non è inclusa nell'ottimizzazione pitch per velocità.
        # L'impatto dei pali è tipicamente < 2% su K_agv e non dipende dal pitch.

        # Irradianza al suolo (W/m²), poi convertita in PAR
        PAR_W_i = compute_irradiance_matrix(f_dir_i, VF_i, dni_ground_opt, dhi_circ_opt,
                                            dhi_horiz_opt, dhi_iso_opt, ghi, albedo)
        PAR_mol_i = PAR_W_i * par_frac_opt[:, None] * W_TO_UMOL
        DLI_h_i = PAR_mol_i * 3600 / 1e6

        dli_df_i = pd.DataFrame(DLI_h_i, index=df_sub.index, columns=x_pts_i)
        dli_daily_i = dli_df_i.groupby(dates_sub).sum()
        dli_mean_i = dli_daily_i.mean()  # media su tutti i giorni, per ogni punto x

        # PAR relativa e RSR per punto
        par_rel_pts = dli_mean_i.values / dli_ref_mean if dli_ref_mean > 0 else np.zeros(n_points)
        rsr_pts = np.clip(1.0 - par_rel_pts, 0, 1)
        y_rel_pts = laub_yield(rsr_pts, target_crop)

        # Media SAU (trapezoidale)
        x_arr_i = np.array(x_pts_i)
        sanu = p_i['sanu']
        sau_mask_i = (x_arr_i >= sanu) & (x_arr_i <= pitch - sanu)
        centr_mask_i = (x_arr_i >= W/2) & (x_arr_i <= pitch - W/2)

        kagv_sau_i = trapz_zone_mean(y_rel_pts, x_arr_i, sau_mask_i) / 100.0
        kagv_centr_i = trapz_zone_mean(y_rel_pts, x_arr_i, centr_mask_i) / 100.0
        par_rel_sau_i = trapz_zone_mean(par_rel_pts, x_arr_i, sau_mask_i)

        results['kagv_sau'][pi] = kagv_sau_i
        results['kagv_centr'][pi] = kagv_centr_i
        results['par_rel_sau'][pi] = par_rel_sau_i
        results['gcr'][pi] = p_i['GCR']

    # Trova ottimale
    print(f'   {n_pitches} pitch valutati.                    ')  # sovrascrive \r
    valid = ~np.isnan(results['kagv_sau'])
    if valid.any():
        best_idx = np.argmax(results['kagv_sau'][valid])
        results['optimal_pitch'] = pitches[valid][best_idx]
        # Segnala se l'ottimo è al bordo superiore del range (K_agv monotono crescente)
        valid_pitches = pitches[valid]
        if abs(results['optimal_pitch'] - valid_pitches[-1]) < step / 2:
            results['at_boundary'] = True
            print(f'   NOTA: pitch ottimale al bordo superiore del range '
                  f'({results["optimal_pitch"]:.2f}m). '
                  f'K_agv potrebbe crescere ulteriormente con pitch maggiori.')
        else:
            results['at_boundary'] = False
    else:
        results['optimal_pitch'] = p_base['pitch']
        results['at_boundary'] = False
...
def self_test():
    ...
    p_test = {
        ...
        'd_palo': 0.0, 'spaziatura_pali': 5.0, 'backtracking': 1,
        'axis_azimuth': 180.0,
    }
