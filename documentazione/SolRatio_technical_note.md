...
Version 4.2.0 completed the planned v4.2 scope, including three items that had originally been scheduled for v4.3 and were brought forward with reduced scope (the "alpha" tier — specular-only transmission for partially-transparent panels — and the "beta" tier — view-factor-only back-side irradiance for bifacial yield). All additions preserve the v4.1 default behaviour when their associated parameters are set to zero or omitted.

**Scientific extensions**

- **Multi-year stochastic mode and P10/P50/P90 quantiles** (`engine/solratio_multiyear.py`): a sequential orchestrator that, given the available PVGIS multi-year time series, executes one annual simulation per year and aggregates the per-KPI distribution. Output is saved incrementally; runs interrupted midway can be resumed.
...
read primarily as a geometric and radiometric result, and as an agronomic estimate
...
![**Figure 4.** Per-crop field-level $K_{\mathrm{agv}}$ (SAU, March–September mean) for the *Sample* (north-south) and *Sample_EW* (east-west) projects with identical geometry, from the `Resa_Colturale` sheets. Shade-sensitive crops are penalised by the east-west axis, while the most shade-penalised categories (grain legumes, C4 maize) benefit slightly from its different seasonal shading distribution.](fig4_kagv_ns_ew.png)
