...
![**Figure 4.** Per-crop field-level $K_{\mathrm{agv}}$ (SAU, March–September mean) for the *Sample* (north-south) and *Sample_EW* (east-west) projects at equal geometry, from the `Resa_Colturale` sheets. Shade-sensitive crops lose with the east-west axis, while the most shade-penalised categories (grain legumes, C4 maize) benefit slightly from its different seasonal shading distribution.](fig4_kagv_ns_ew.png)
